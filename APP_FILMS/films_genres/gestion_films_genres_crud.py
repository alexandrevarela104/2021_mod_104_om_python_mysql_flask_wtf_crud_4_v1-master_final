...
@obj_mon_application.route("/films_genres_afficher/<int:id_genre_sel>", methods=['GET', 'POST'])
def films_genres_afficher(id_genre_sel):
    if request.method == "GET":
        try:
            try:
                # Renvoie une erreur si la connexion est perdue.
                MaBaseDeDonnee().connexion_bd.ping(False)
            except Exception as Exception_init_motifs_genres_afficher:
                code, msg = Exception_init_motifs_genres_afficher.args
                flash(f"{error_codes.get(code, msg)} ", "danger")
                flash(f"Exception _init_motifs_genres_afficher problème de connexion BD : {sys.exc_info()[0]} "
                      f"{Exception_init_motifs_genres_afficher.args[0]} , "
                      f"{Exception_init_motifs_genres_afficher}", "danger")
                raise MaBdErreurConnexion(f"{msg_erreurs['ErreurConnexionBD']['message']} {erreur.args[0]}")

            with MaBaseDeDonnee().connexion_bd.cursor() as mc_afficher:
                strsql_genres_films_afficher_data = """   SELECT id_motif, motif,
                                                            GROUP_CONCAT(nom_user) as GenresFilms FROM t_motif
                                                            RIGHT JOIN t_facture ON t_facture.id_facture = t_motif.fk_facture
                                                            LEFT JOIN t_user ON t_user.id_user = t_motif.fk_user
                                                             GROUP BY id_motif"""
                if id_genre_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING id_motif= %(value_id_genre_selected)s"""

                    mc_afficher.execute(strsql_genres_films_afficher_data, valeur_id_genre_selected_dictionnaire)
                ...
                if not data_genres_films_afficher and id_genre_sel == 0:
                    flash("""La table "t_motif" est vide. !""", "warning")
                elif not data_genres_films_afficher and id_genre_sel > 0:
                    # Si l'utilisateur change l'id_motif dans l'URL et qu'il ne correspond à aucun film
...
@obj_mon_application.route("/edit_motif_selected", methods=['GET', 'POST'])
def edit_motif_selected():
    if request.method == "GET":
        try:
            with MaBaseDeDonnee().connexion_bd.cursor() as mc_afficher:
            ...
            print("dans edit_motif_selected ---> data_genres_all", data_genres_all)

            # Récupère la valeur de "id_motif" du formulaire html "films_genres_afficher.html"
            # l'utilisateur clique sur le bouton "Modifier" et on récupère la valeur de "id_motif"
            # grâce à la variable "id_motif_genres_edit_html" dans le fichier "films_genres_afficher.html"
            # href="{{ url_for('edit_motif_selected', id_motif_genres_edit_html=row.id_motif) }}"
            id_motif_genres_edit = request.values['id_motif_genres_edit_html']

            # Mémorise l'id du film dans une variable de session
            # (ici la sécurité de l'application n'est pas engagée)
            # il faut éviter de stocker des données sensibles dans des variables de sessions.
            session['session_id_motif_genres_edit'] = id_motif_genres_edit

            # Constitution d'un dictionnaire pour associer l'id du film sélectionné avec un nom de variable
            valeur_id_genre_selected_dictionnaire = {"value_id_genre_selected": id_motif_genres_edit}
            ...
            data_genre_film_selected, data_genres_films_non_attribues, data_genres_films_attribues = \
                genres_films_afficher_data(valeur_id_genre_selected_dictionnaire)

            print(data_genre_film_selected)
            lst_data_film_selected = [item['id_motif'] for item in data_genre_film_selected]
            ...
            # Extrait les valeurs contenues dans la table "t_users", colonne "intitule_genre"
            # Le composant javascript "tagify" pour afficher les tags n'a pas besoin de l'id_motif
            lst_data_genres_films_non_attribues = [item['nom_user'] for item in data_genres_films_non_attribues]
            print("lst_all_genres gf_edit_motif_selected ", lst_data_genres_films_non_attribues,
                  type(lst_data_genres_films_non_attribues))

        except Exception as Exception_edit_motif_selected:
            code, msg = Exception_edit_motif_selected.args
            flash(f"{error_codes.get(code, msg)} ", "danger")
            flash(f"Exception edit_motif_selected : {sys.exc_info()[0]} "
                  f"{Exception_edit_motif_selected.args[0]} , "
                  f"{Exception_edit_motif_selected}", "danger")
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            id_genre_selected = session['session_id_motif_genres_edit']
            print("session['session_id_motif_genres_edit'] ", session['session_id_motif_genres_edit'])
            ...
            new_lst_int_motif_old = list(map(int, new_lst_str_genres_films))
            print("new_lst_motif ", new_lst_int_motif_old, "type new_lst_motif ",
                  type(new_lst_int_motif_old))

            # Pour apprécier la facilité de la vie en Python... "les ensembles en Python"
            # https://fr.wikibooks.org/wiki/Programmation_Python/Ensembles
            # OM 2021.05.02 Une liste de "id_motif" qui doivent être effacés de la table intermédiaire "t_motif".
            lst_diff_genres_delete_b = list(
                set(old_lst_data_genres_films_attribues) - set(new_lst_int_motif_old))
            print("lst_diff_genres_delete_b ", lst_diff_genres_delete_b)

            # Une liste de "id_motif" qui doivent être ajoutés à la "t_motif"
            lst_diff_genres_insert_a = list(
                set(new_lst_int_motif_old) - set(old_lst_data_genres_films_attribues))
            print("lst_diff_genres_insert_a ", lst_diff_genres_insert_a)

            # SQL pour insérer une nouvelle association entre
            # "fk_facture"/"id_motif" et "fk_user"/"id_motif" dans la "t_motif"
            strsql_insert_motif = """INSERT INTO t_motif (id_motif, fk_user, fk_facture)
                                                    VALUES (NULL, %(value_fk_user)s, %(value_fk_facture)s)"""

            # SQL pour effacer une (des) association(s) existantes entre "id_motif" et "id_motif" dans la "t_motif"
            strsql_delete_genre_film = """DELETE FROM t_motif WHERE fk_user = %(value_fk_user)s AND fk_facture = %(value_fk_facture)s"""

            with MaBaseDeDonnee() as mconn_bd:
                # Pour le film sélectionné, parcourir la liste des genres à INSÉRER dans la "t_motif".
                # Si la liste est vide, la boucle n'est pas parcourue.
                for id_motif_ins in lst_diff_genres_insert_a:
                    # Constitution d'un dictionnaire pour associer l'id du film sélectionné avec un nom de variable
                    # et "id_motif_ins" (l'id du genre dans la liste) associé à une variable.
                    valeurs_film_sel_genre_sel_dictionnaire = {"value_fk_facture": id_genre_selected,
                                                               "value_fk_user": id_motif_ins}

                    mconn_bd.mabd_execute(strsql_insert_motif, valeurs_film_sel_genre_sel_dictionnaire)

                # Pour le film sélectionné, parcourir la liste des genres à EFFACER dans la "t_motif".
                # Si la liste est vide, la boucle n'est pas parcourue.
                for id_motif_del in lst_diff_genres_delete_b:
                    # Constitution d'un dictionnaire pour associer l'id du film sélectionné avec un nom de variable
                    # et "id_motif_del" (l'id du genre dans la liste) associé à une variable.
                    valeurs_film_sel_genre_sel_dictionnaire = {"value_fk_facture": id_genre_selected,
                                                               "value_fk_user": id_motif_del}
    ...
    # Après cette mise à jour de la table intermédiaire "t_motif",
    # on affiche les films et le(urs) genre(s) associé(s).
    return redirect(url_for('films_genres_afficher', id_genre_sel=id_genre_selected))
...
def genres_films_afficher_data(valeur_id_genre_selected_dict):
    print("valeur_id_genre_selected_dict...", valeur_id_genre_selected_dict)
    try:

        strsql_film_selected = """SELECT id_motif, motif, GROUP_CONCAT(id_user) as GenresFilms FROM t_motif
                                        INNER JOIN t_facture ON t_facture.id_facture = t_motif.fk_facture
                                        INNER JOIN t_user ON t_user.id_user = t_motif.fk_user
                                        WHERE id_motif = %(value_id_genre_selected)s"""

        strsql_genres_films_non_attribues = """SELECT id_user, nom_user FROM t_user WHERE id_user not in(SELECT id_user as idGenresFilms FROM t_motif
                                                    INNER JOIN t_facture ON t_facture.id_facture = t_motif.fk_facture
                                                    INNER JOIN t_user ON t_user.id_user = t_motif.fk_user
                                                    WHERE id_motif = %(value_id_genre_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_motif, motif FROM t_motif
                                            INNER JOIN t_facture ON t_facture.id_facture = t_motif.fk_facture
                                            INNER JOIN t_user ON t_user.id_user = t_motif.fk_user
                                            WHERE id_motif = %(value_id_genre_selected)s"""
        ...
        with MaBaseDeDonnee().connexion_bd.cursor() as mc_afficher:
            ...
            # Envoi de la commande MySql
            mc_afficher.execute(strsql_film_selected, valeur_id_genre_selected_dict)
            # Récupère les données de la requête.
            ...
            # Envoi de la commande MySql
            mc_afficher.execute(strsql_genres_films_attribues, valeur_id_genre_selected_dict)
            # Récupère les données de la requête.
